...
def base_render_template(template, data_id, **kwargs):
    ...
    curr_filtered_ranges = {}
    if final_query == curr_settings.get("filteredRanges", {}).get("query"):
    ...
    return render_template(
        ...
        filtered_ranges=json.dumps(curr_filtered_ranges),
...
@exception_decorator
def update_filtered_ranges(data_id):
    curr_settings = global_state.get_settings(data_id) or {}
    final_query = build_query(data_id, curr_settings.get("query"))
    if not final_query:
        return {}
